#!/bin/bash
# Third batch: the frame-based engines that do NOT need interpolation.
# This is the fairest comparison, since Spacer is itself PDR + model-based
# projection and solves the same problems in CHC form in ~0.04 s.
...
for task in sum_linear fib_n5_eq5 fib_n5_eq6 fib_nonneg mc91_n87 mc91_any; do
  for alg in ic3 pdr; do for slv in z3 msat; do echo "$task $alg $slv"; done; done
