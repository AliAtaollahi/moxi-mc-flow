...
TASKS=$(cd $BASE/out && ls x*.moxi | sed 's/\.moxi$//')
for task in $TASKS; do
  for alg in bmc kind imc ismc ic3ia; do echo "$task $alg msat"; done
done | xargs -P 10 -L 1 bash -c 'run $0 $1 $2'
echo "DONE7"
